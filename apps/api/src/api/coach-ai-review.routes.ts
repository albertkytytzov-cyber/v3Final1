...
export function registerCoachAiReviewRoutes(
  ...
  app.get("/api/v1/coach/ai-period-reviews", async (request) => {
    const user = await requireCoachAiReviewUser(request);

    return {
      reviews: await listCoachPeriodAiReviewsForCoachContext({
        coachUserId: user.id,
        role: user.role,
      }),
    };
  ...
  app.post("/api/v1/coach/athletes/:athleteId/ai-period-review", async (request) => {
    const user = await requireCoachAiReviewUser(request);
    ...
    let payload;
    try {
      payload = parseCoachPeriodAiReviewBody(request.body);
    } catch (error) {
      throw dependencies.httpError(400, (error as Error).message);
    }
    ...
    return {
      review: await saveCoachPeriodAiReview({
        coachUserId: user.id,
        review,
      }),
    };
