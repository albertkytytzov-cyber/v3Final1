import type { ConstructorDraft, PlanTemplatePayload } from "@training-platform/shared";
import { buildConstructorTemplatePayload } from "@training-platform/shared";
...
function flattenSessions(payload?: PlanTemplatePayload | null) {
  return payload?.days?.flatMap((day) => day.sessions) ?? [];
}

function flattenBlocks(payload?: PlanTemplatePayload | null) {
  return flattenSessions(payload).flatMap((session) => session.blocks);
}

function flattenExercises(payload?: PlanTemplatePayload | null) {
  return flattenBlocks(payload).flatMap((block) => block.exercises ?? []);
}

function containsControlColumnMarker(payload?: PlanTemplatePayload | null) {
  const candidates = [
    ...(payload?.blocks ?? []),
...
function containsInternalMatrixFields(payload?: PlanTemplatePayload | null) {
  if (!payload) {
    return false;
  }
...
function summarizePayload(payload: PlanTemplatePayload): MatrixPrimaryPilotSaveDryRunSummary {
...
export function buildMatrixPrimaryPilotSaveDryRun(params: {
  ...
  let payload: PlanTemplatePayload | null = null;
  let buildError = "";
