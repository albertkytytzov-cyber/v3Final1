...
- если часы начали отдавать activity-файл, но остановились на частичном пакете
  (`partial-timeout`) или CRC не сошелся, native слой делает один повтор того же
  файла в текущей очереди и оставляет более полный результат;
