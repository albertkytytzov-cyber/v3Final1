...
ИИ-разбор в Review не меняет план, дневник и отметки спортсмена. Он сохраняет только историю выводов и помогает тренеру быстрее понять, что произошло в выбранный день и что происходит по периоду подготовки.

В Review есть два разных разбора:

- **Разбор дня** - оценивает только выбранную дату: качество данных, готовность, план, выполнение, данные устройства и запись тренера.
- **Общий разбор периода** - оценивает 7, 14 или 30 дней: накопленную нагрузку, выполнение, готовность, сон, пульс покоя, вес и тренировки с устройства.

Эти два сценария не смешиваются. Дневной разбор помогает понять конкретный день, а общий разбор нужен для решения по микроциклу или текущему тренировочному отрезку.

Структура дневного разбора:

- **Главное решение** - короткий тренерский вывод: день зачтён, осторожно, риск или нужно вмешательство.
- **Уверенность вывода** - показывает, насколько полно собраны данные. Если нет готовности, сна, пульса покоя, SpO2 или комментария тренера, вывод считается ограниченным.
- **Методический слой** - отдельные показатели для борьбы: энергетика, локальная нагрузка, контакт и техника.
- **Что видно** - краткое описание факта выполнения.
- **Чего не хватает для точности** - чеклист данных, которые нужно добавить или синхронизировать.
- **Анализ по блокам** - связь блока плана с intent, энергетикой, локальными зонами и риском.
- **Что сделать завтра** - решение в формате: оставить, проверить, не добавлять, решить после данных.

Структура общего разбора:

- **Вывод по периоду** - короткий общий вывод по выбранному окну.
- **Метрики периода** - план/факт за 7 дней, средняя готовность, средний сон, динамика веса.
- **Тренды** - нагрузка, готовность, пульс покоя, сон.
- **Риски** - накопленная усталость, конфликт нагрузки и восстановления, неполные данные, незакрытые дни.
- **Решения тренера по периоду** - что делать со следующим микроциклом или ближайшими днями.

История дневных и общих разборов показывается отдельно.

Если появляется предупреждение **Разбор устарел**, это не ошибка. Оно означает, что после сохранения разбора изменился план, факт, данные устройства, набор доступных показателей или методика анализа. Перед решением нажмите **Обновить ИИ-разбор**.

Контекст весогонки учитывается только при явных признаках: сгонка, weight cut, весогонка, стартовая масса, ограничение воды, соли или углеводов. Обычное наличие весовой категории само по себе не считается весогонкой.
