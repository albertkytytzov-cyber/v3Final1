...
## 41. Stage 30: Trainer-facing matrix pilot surface and save/assign gate

Stage 30 begins the move from engineering QA to coach-facing controlled use.

It adds two things:

1. A trainer-facing Russian UI layer for the matrix panel.
2. A separate explicit save/assign pilot flag for future real template usage.

### 41.1 Trainer-facing UI language

The internal panel no longer exposes coach-facing Russian labels such as
`legacy draft`, `matrix preview`, `rollout decision`, `workspace`, `read-only`,
or `dry-run`.
...
- `решение по применению`;
- `готовность к применению`;
- `проверка безопасного сохранения`;
- `новый вариант плана`.

The underlying code-level terms remain unchanged. This is intentionally only a
UI clarity layer.

### 41.2 Save/assign pilot flag

Stage 30 adds a third build-time web flag:

```bash
NEXT_PUBLIC_MATRIX_CONSTRUCTOR_SAVE_ASSIGN_PILOT=true
```

It is ignored unless both earlier flags are enabled:
...
Default remains false in `.env.example`.

### 41.3 Guarded save path

The normal constructor save path remains legacy-capable through
`isConstructorDraftSaveAllowed("legacy")`.

For a matrix draft, the UI can prepare a template payload only when all of the
following are true:

- internal matrix UI flag is enabled;
- limited primary pilot flag is enabled;
- save/assign pilot flag is enabled;
- active source is `matrix_primary_pilot`;
- local primary pilot eligibility passed;
- server primary pilot gate passed;
- local save dry-run status is `passed`;
- server save dry-run status is `passed`;
- active draft was generated by matrix.
...
- production `/api/v1/plans/constructor/draft`;
- rollout policy;
- matrix draft generation rules;
- DB schema;
- mobile contracts;
- telemetry/storage;
- `matrix_internal` read-only behavior.

`matrix_primary_pilot` remains blocked from the old generic
`isConstructorDraftSaveAllowed(...)` helper. The new guarded path is separate
and requires the third explicit flag plus server evidence.
...
`check:constructor-matrix-ui-gates` now verifies:

- all three flags default to false;
- save/assign pilot cannot enable without internal UI + limited pilot;
- legacy remains save-capable;
...
## 53. Stage: Evidence Claim Blocker Review Intake Pack

Stage: Evidence Claim Blocker Review Intake Pack.

This stage adds a metadata-only review intake path for every evidence claim
blocker:
...
- shared exports for intake ids and summary helpers;
- Review Package review-intake summary metadata.

The intake pack does not approve claims, does not extract claims and does not
create numeric thresholds. It does not invent citations, source metadata,
reviewer names or review dates. Every intake remains `humanReviewed=false`, and
...
text before extraction. Human-review blockers require real coach, medical,
data-quality, sport-science or product-safety decisions before future
extraction.
