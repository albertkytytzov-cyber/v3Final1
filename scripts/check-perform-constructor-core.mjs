...
const competitionWeekSpeedDays = competitionWeekDraft.plan.weeks
  .flatMap((week) => week.days.map((day) => ({ ...day, phase: week.phase })))
  .filter((day) => day.blocks.some((block) => block.targetQuality === "speed_first_action"));
...
assert(
  templatePayload.days.some((day) =>
    day.sessions.some((session) => session.blocks.some((block) => block.blockType === "speed")),
  ),
  "Generated plan should include speed work for speed_first_action goal",
);
assert(
  totalDraftDays >= 15,
...
assert(competitionWeekSpeedDays.length > 0, "Competition week should include short speed activation");
assert(
  competitionWeekSpeedDays.every(
    (day) =>
      day.blocks.length >= 3 &&
      day.blocks.some((block) => block.targetQuality === "fatigue_skill") &&
      day.blocks.some((block) => block.targetQuality === "weight_management"),
  ),
  "Speed days in taper/start week must include wrestling transfer and weight/recovery control, not only accelerations",
);
  ...
  JSON.stringify(
    {
      ...
      competitionWeekSpeedDays: competitionWeekSpeedDays.map((day) => ({
        label: day.dayLabel,
